...
parser.add_argument('--lrD', type=float, default=5e-5, help='learning rate for Critic, default=0.00005')
parser.add_argument('--lrG', type=float, default=2e-3, help='learning rate for Generator, default=0.00005')
...
######################################
def calc_gradient_penalty(netD, real_data, fake_data, lmda):
    batch_size = real_data.size(0)
    epsilon = torch.rand(batch_size, 1)
    epsilon = epsilon.expand(batch_size, real_data.nelement()/batch_size).contiguous().view(batch_size, real_data.size(1), real_data.size(2), real_data.size(3))
    ...
    gradient_penalty = ((gradients.norm(2, dim=1) - 1) ** 2).mean() * lmda
...
while epoch < opt.max_epochs:
    ...
    while i < len(dataloader): # running one epoch 
        ...
        while j < Diters and i < len(dataloader):
            ...
            real_labels = torch.FloatTensor(1).fill_(1)
            fake_labels =  torch.FloatTensor(1).fill_(-1) 

            if opt.cuda:
                real_labels, fake_labels = Variable(real_labels.cuda()), Variable(fake_labels.cuda())
                data_real = data_real.cuda()
            
            inputv = Variable(data_real)

            netD.zero_grad()
            errD_real = netD(inputv)
            errD_real.backward(real_labels)
            ...
            errD_fake.backward(fake_labels)
